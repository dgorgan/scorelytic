import {
  evaluateBiasImpact,
  generateBiasReport,
} from "@/shared/utils/biasAdjustment";
...
describe("evaluateBiasImpact", () => {
  ...
  it("clamps score to 0-10", () => {
    expect(evaluateBiasImpact(9.9, ["influencer bias"]).biasAdjustedScore).toBe(
      10,
    );
...
describe("generateBiasReport", () => {
  it("produces all output layers for classic and ideological biases", () => {
    const { summary, details, culturalContext, fullReport } =
      generateBiasReport(8.5, [
        "nostalgia bias",
    ...
    expect(details.some((d) => d.name === "identity signaling bias")).toBe(
      true,
    );
    ...
    expect(fullReport.score_analysis_engine.bias_adjusted_score).toBe(
      summary.adjustedScore,
    );
    expect(
      fullReport.score_analysis_engine.ideological_biases_detected.length,
    ).toBe(4);
  });

  it("handles no bias", () => {
    const { summary, details, culturalContext, fullReport } =
      generateBiasReport(7, []);
