...
const fieldStats: Record<string, { total: number, exact: number, high: number, mismatches: { seed: string, llm: string, sim: string }[] }> = {};
...
console.log(`Field Reliability Summary (arrayThresh=${arrayThresh}, stringThresh=${stringThresh}):`);
...
for (const field of allFields) {
  const s = fieldStats[field] || { total: 0, exact: 0, high: 0, mismatches: [] };
  let mismatchStr = '';
  if (s.mismatches.length === 0) {
    mismatchStr = '(no mismatches)';
  } else {
    mismatchStr = s.mismatches.slice(0,5).map(m => {
      let note = '';
      if (m.seed === '[]' && m.llm === '[]' && m.sim === '1.00') note = ' (both empty arrays, perfect match)';
      return `seed: ${m.seed}\nllm:  ${m.llm}\nsim:  ${m.sim}${note}`;
    }).join('\n---\n');
  }
  console.log(`${field.padEnd(15)} | ${s.total.toString().padEnd(5)} | ${s.exact.toString().padEnd(5)} | ${s.high.toString().padEnd(7)} | ${mismatchStr}`);
} 