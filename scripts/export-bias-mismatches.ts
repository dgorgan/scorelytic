...
const outRows = ['reviewId,seed,llm,similarity'];
const htmlRows = [
...
for (const line of lines) {
  ...
  htmlRows.push(
    `<tr><td>${reviewId}</td><td>${seed}</td><td>${llm}</td><td>${similarity}</td></tr>`,
  );
}
htmlRows.push('</table>');
fs.writeFileSync('bias_mismatches.csv', outRows.join('\n'));
fs.writeFileSync(
  'bias_mismatches.html',
  `<html><body><h2>BiasIndicators Mismatches</h2>${htmlRows.join('')}</body></html>`,
);
