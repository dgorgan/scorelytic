...
export const FEW_SHOT_EXAMPLES = [
  {
    transcript: "Dragon Age: The Veilguard surprised me. As someone who loved Origins, this felt like a return to meaningful party-based storytelling. Boss fights felt deliberate, though trash mobs wore thin. Visually, it's the best the series has ever looked. I missed more meaningful consequences from my Inquisition save, though.",
    expected: {
      sentimentScore: 8.5,
      verdict: "positive",
      ...
      reviewSummary: "A rewarding RPG that revives the series' strengths, even if legacy choices take a backseat."
    }
  }
];
...
export const analyzeText = async (
  ...
  const shouldUseAlternativePrompt = model.includes('gpt-3.5') || process.env.LLM_PROMPT_STYLE === 'ALTERNATIVE';
  const promptToUse = customPrompt || (shouldUseAlternativePrompt ? UPDATED_LLM_PROMPT_ALTERNATIVE : UPDATED_LLM_PROMPT);
  ...
  for (const chunk of transcriptChunks) {
    ...
    if (
      ...
      console.warn('[LLM] Primary prompt failed or missing summary — retrying with alternative prompt.');
  ...
  const toStringArray = (v: any) => Array.isArray(v) ? v.filter(x => typeof x === 'string') : [];
  ...
  const aggregate = (field: keyof SentimentResult, isArray = false): string[] | string | null => {
    if (isArray) {
      ...
      return toStringOrNull(results.find(r => r[field] && r[field] !== 'No clear summary detected.')?.[field]);
    }
  };

  const sentimentScore = results.find(r => typeof r.sentimentScore === 'number')?.sentimentScore ?? 5;
  ...
  const result: SentimentResult = {
    summary:
      toStringOrNull(summary) ||
      toStringOrNull(sentimentSummary) ||
      'No clear summary detected.',
    sentimentScore: typeof sentimentScore === 'number' && sentimentScore >= 0 && sentimentScore <= 10 ? sentimentScore : 5,
  ...
  if (!result.summary || result.summary === 'No clear summary detected.') {
    console.warn('[LLM] Warning: summary is missing, using fallback (sentimentSummary or default).');
  }
...
export const mapBiasLabelsToObjects = (
  ...
  return biasLabels.map(label => {
    ...
    if (label === 'nostalgia bias') {
      ...
      severity = 'high';
      impactOnExperience = 'Potential for inflated praise or downplaying negatives due to external incentives.';
      scoreInfluence = 1.0;
      ...
      severity = 'high';
      impactOnExperience = 'Narrative framing may enhance resonance for aligned players, but compromise immersion for others.';
      scoreInfluence = -0.3;
      ...
      severity = 'moderate';
      impactOnExperience = 'Emphasis on representation may be valued by some, but seen as forced by others.';
      scoreInfluence = -0.2;
...
export const analyzeBiasImpact = async (
  ...
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        { role: 'system', content: 'You are an expert in bias analysis and score adjustment for game reviews.' },
    ...
    try {
      ...
      const biasObjs = mapBiasLabelsToObjects(payload.biasIndicators, payload.reviewSummary, payload.pros, payload.cons);
      const totalScoreAdjustment = biasObjs.reduce((sum, b) => sum + b.scoreInfluence, 0);
      return {
        originalScore: payload.originalScore,
        biasAdjustedScore: Math.max(0, Math.min(10, Math.round((payload.originalScore + totalScoreAdjustment) * 10) / 10)),
        totalScoreAdjustment,
        biasAnalysis: biasObjs,
        audienceFit: 'General gaming audience; see bias analysis for context.',
        adjustmentRationale: 'Fallback: Score adjusted based on detected biases and simple heuristics.'
      };
    }
    // Clamp score to [0, 10]
...
export const analyzeTextWithBiasAdjustmentFull = async (
  ...
  const sentiment = await analyzeText(text, preferredModel, customPrompt, labels, gameTitle, creatorName);
  // Bias detection phase
  const biasDetection: BiasDetectionPhaseOutput = {
    originalScore: sentiment.sentimentScore ?? 5,
    biasesDetected: mapBiasLabelsToObjects(sentiment.biasIndicators, sentiment.reviewSummary || '', sentiment.pros, sentiment.cons),
  ...
  const confidenceLevel: 'low' | 'moderate' | 'high' = Math.abs((sentiment.sentimentScore ?? 5) - 5) > 2 ? 'high' : 'moderate';
  const recommendationStrength: 'low' | 'moderate' | 'strong' = (sentiment.sentimentScore ?? 5) > 7 ? 'strong' : (sentiment.sentimentScore ?? 5) > 5 ? 'moderate' : 'low';
  ...
  const culturalContext: CulturalContextExplanation = {
    justification: 'Based on the review transcript, certain ideological themes and narrative framings may influence audience perception.',
    ideologicalThemes: ['representation', 'studio reputation'],
    audienceReactions: {
      aligned: 'Likely to resonate strongly with fans of inclusive narratives and franchise loyalists.',
...
export const MOCK_FULL_BIAS_SCORING_OUTPUT: FullBiasScoringOutput = {
  sentiment: {
    summary: 'Dragon Age: The Veilguard revitalizes BioWare\'s RPG legacy with its stunning visuals, engaging storytelling, and rich character development, though some combat aspects may disappoint tactical purists.',
    sentimentScore: 9.2,
    verdict: 'positive',
