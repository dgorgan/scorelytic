...
describe('transcribeYouTubeAudio', () => {
  ...
  it('should transcribe audio successfully', async () => {
    ...
    const result = await audioService.transcribeYouTubeAudio('videoId', { maxDurationMinutes: 30 }, helpers);
  ...
  it('should handle yt-dlp failure', async () => {
    ...
    await expect(audioService.transcribeYouTubeAudio('videoId', { maxDurationMinutes: 30 }, helpers)).rejects.toThrow('yt-dlp fail');
  });

  it('should handle OpenAI API failure', async () => {
    helpers.downloadAudio.mockResolvedValue(undefined);
    helpers.transcribeAudioWithOpenAI.mockRejectedValue(new Error('OpenAI fail'));
    await expect(audioService.transcribeYouTubeAudio('videoId', { maxDurationMinutes: 30 }, helpers)).rejects.toThrow('OpenAI fail');
  });
}); 