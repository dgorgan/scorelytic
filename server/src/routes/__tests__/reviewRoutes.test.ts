...
jest.mock('../../services/sentiment', () => ({
  analyzeSentiment: jest.fn(async (text: string) => ({ summary: 'R', sentimentScore: 2, verdict: 'negative' }))
}));
...
describe('reviewRoutes', () => {
  ...
  it('404s if review not found', async () => {
    jest.spyOn(require('../../services/youtube/reviewMetadataService'), 'getReviewMetadata').mockResolvedValueOnce(null);
    const res = await request(app)
      .post('/api/reviews/analyze')
      .send({ reviewId: '404' });
  ...
  it('400s if no transcript', async () => {
    jest.spyOn(require('../../services/youtube/reviewMetadataService'), 'getReviewMetadata').mockResolvedValueOnce({
      gameTitle: 'Sample Game',
