...
import { Result, displayCell, prettyTooltip } from '@/components/dashboard/utils';
    ...
    <div className="overflow-x-auto mt-8 max-w-full">
      <table className="w-full min-w-max border border-gray-300 rounded-lg shadow text-sm bg-white">
        <thead className="bg-neutral-100 sticky top-0 z-10">
          <tr>
            <th className="px-4 py-2 border-b font-bold text-gray-900 w-[120px] max-w-[300px]">
              Review ID
            </th>
            {reviewFields.map((field) => (
              <th
                key={field}
                className="px-4 py-2 border-b font-bold text-gray-900 w-[180px] max-w-[300px]"
              >
                {field}
              </th>
            ))}
        ...
        <tbody>
          {filteredGroupedResults.map(
            (row: GroupedResult & { hasMismatch: boolean }, i: number) => (
              <tr
                key={row.reviewId}
                className={
                  row.hasMismatch
              ...
              >
                <td className="px-4 py-2 border-b w-[120px] max-w-[300px] text-black">
                  {row.reviewId}
                </td>
                {reviewFields.map((field: string) => (
                  <td
                    key={field}
                    className="px-4 py-2 border-b w-[180px] max-w-[300px] text-black align-top"
                  >
                    <div className="bg-gray-50 rounded p-2 mb-1 border border-gray-200">
                      ...
                      <Tooltip.Root delayDuration={200}>
                        ...
                        <Tooltip.Portal>
                          <Tooltip.Content
                            className="z-50 max-w-xs rounded bg-gray-900 text-white px-3 py-2 text-xs shadow-lg whitespace-pre-line"
                            side="top"
                            align="center"
                          >
